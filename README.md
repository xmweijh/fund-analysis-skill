...
- 📦 **批量持仓分析**：一键分析所有持仓基金，报告含**总结摘要表**（操作建议/涨跌幅/净值/收益一览）和**可跳转目录**
...
## 🗂 持仓总结          ← 所有持仓一览表（可点击基金名跳转）
| # | 基金 | 操作建议 | 日涨跌幅 | 当前净值 | 近1年收益 | 核心结论 |
> 📌 操作分布：买入 N只，持有 N只

## 📑 目录              ← 带操作建议/涨跌幅的锚点目录
1. [基金名 `代码`](#fund-XXXXXX) — 买入（-0.58%）
...

## 📋 持仓列表          ← 成本等基础信息

---
各基金完整详细报告（标题含 HTML 锚点，支持目录跳转）
```
